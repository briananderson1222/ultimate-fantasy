...
import { getPublicLeague, getScoreboard, getLeagueMembers } from "../../../services/api";
    ...
    <main className="min-h-screen p-6">
      <LeagueBranding themeVars={info.data?.branding?.theme}>
        <div className="mx-auto max-w-3xl space-y-6">
          ...
          {info.isError && <p className="text-sm text-red-600">{(info.error as Error)?.message}</p>}
          {info.data && (
            <Card>
              <div className="text-lg font-medium mb-1">
                {info.data.branding?.name || info.data.name}
              </div>
              ...
              {tab === "overview" && (
                <div className="text-sm text-[var(--color-text)]">
                  Welcome to the league. Use the actions below to join and participate.
                </div>
              )}
              {tab === "scoreboard" && (
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
                      {(() => {
                        ...
                        for (const it of items)
                          current.set(String(it.team_id), it.total_points ?? 0);
                        const top = items.length
                          ? Math.max(...items.map((i) => i.total_points ?? 0))
                          : 0;
                            ...
                            <div key={idx} className="surface rounded-[var(--radius-md)] p-3">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="text-right">
                                  ...
                                  <div
                                    className={`flex items-center justify-end gap-1 text-xs ${trendColor}`}
                                    aria-live="polite"
                                  >
                      ...
                      {scoreboard.data.items.length === 0 && (
                        <div className="sm:col-span-2 md:col-span-3">
                          <EmptyState
                            title="No scores yet"
                            description="Lineups will appear here once games are played."
                          />
                        </div>
                      )}
                      {(() => {
                        ...
                        const items = scoreboard.data!.items;
                        items.forEach((it) =>
                          prevPointsRef.current.set(String(it.team_id), it.total_points ?? 0),
                        );
                        return null;
                      })()}
                    </div>
                          ...
                          <div key={m.team_id} className="surface rounded-[var(--radius-md)] p-3">
                            <div className="flex items-center gap-3">
                              <div
                                className="flex h-10 w-10 items-center justify-center rounded-full text-white shadow"
                                style={{
                                  background: `linear-gradient(135deg, hsl(${h} 70% 35%), hsl(${(h + 30) % 360} 70% 45%))`,
                                }}
                                aria-hidden
                              >
                                <span className="text-sm font-semibold">
                                  {(m.team_name || "T").slice(0, 2).toUpperCase()}
                                </span>
                              </div>
                      ...
                      {members.data.items.length === 0 && (
                        <div className="sm:col-span-2 md:col-span-3">
                          <EmptyState
                            ...
                            description="Be the first to join this league."
                          />
                        </div>
                      )}
                    </div>
          ...
          <Card className="space-y-3">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="text-xs font-mono bg-[var(--color-surface)] border border-[var(--border)] rounded px-2 py-1 flex-1 min-w-0"
                  />
                  <Button
                    size="sm"
                    onClick={() => {
                      if (typeof window !== "undefined") {
                        ...
                        toast.show({
                          title: "Copied!",
                          description: "League URL copied to clipboard",
                        });
                      }
                    }}
                  >
            ...
            <p className="text-sm text-[var(--color-muted)]">
              Share this URL with others to invite them to join the league.
            </p>
          </Card>
              ...
              <ul className="text-sm text-[var(--color-text)]">
                {(() => {
                  ...
                  return items.map((it, idx) => (
                    <li
                      key={idx}
                      className="flex items-center justify-between border-b border-[var(--border)] py-1 hover:bg-[var(--color-elevated)]"
                    >
                      <span className="text-gray-600">Team {String(it.team_id).slice(0, 8)}</span>
                      <span className="flex items-center gap-2">
                        {top > 0 && (it.total_points ?? 0) === top && (
                          <Badge variant="success">Leader</Badge>
                        )}
                ...
                {scoreboard.data.items.length === 0 && (
                  <li className="py-2">
                    <EmptyState
                      title="No scores yet"
                      description="Come back after lineups are scored."
                    />
                  </li>
                )}
              </ul>
          ...
          <OnboardingTour
            ...
            steps={[
              {
                selector: '[data-tour="join-button"]',
                title: "Join the league",
                content: "Join to participate in matchups and scoring.",
              },
              {
                selector: "#league-tabs",
                title: "Explore tabs",
                content: "Overview, Scoreboard, and Managers tabs show league info.",
              },
            ]}
            open={tourOpen}
