...
import { listLineups, getMyLeagues, getLeagueMembers, type LineupListResponse, type MembersResponse, type MeLeaguesResponse } from "../../../services/api";
    ...
    <div className="min-h-screen p-6">
      <div className="mx-auto max-w-4xl space-y-6">
        <PageHeader
          ...
          actions={
            leagueId ? (
              <Link className="text-blue-700 underline px-2 py-1 rounded focus-visible:ring-2 focus-visible:ring-offset-2" href={`/leagues/${leagueId}/matchups`}>
                View Matchups
              </Link>
            ) : null
          }
        ...
        <Card className="space-y-3">
          ...
          {lineups.isError && (
            <p className="text-sm text-red-600">{lineups.error.message}</p>
          )}
          {latest && (
            <ul className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
              {latest.players.map((p, idx) => (
                <li key={`${p.player_id}-${idx}`} className="surface rounded-[var(--radius-md)] p-3">
                  <div className="flex items-center justify-between">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium">{String(p.player_id).slice(0, 8)}</div>
                      <div className="text-xs text-[var(--color-muted)]">ID: {String(p.player_id)}</div>
                    </div>
          ...
          {!latest && !lineups.isLoading && (
            <EmptyState title="No roster yet" description="Save a lineup to populate your roster." />
          )}
        </Card>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card className="space-y-2">
            ...
            {!leagueId && (
              <p className="text-sm text-gray-600">Link a league to see matchups.</p>
            )}
                ...
                {upcoming.opponent ? (
                  <div className="flex items-center justify-between">
                    <div className="text-sm">
                      ...
                      <div className="text-gray-600">Team {String(teamId).slice(0, 6)} vs Team {String(upcoming.opponent).slice(0, 6)}</div>
                    </div>
                    <Link className="text-blue-700 underline" href={`/leagues/${leagueId}/matchups`}>Details</Link>
                  </div>
                ) : (
          ...
          <Card className="space-y-2">
            ...
            <div className="rounded border p-3">
              ...
              <div className="mt-2 text-xs text-gray-500">Based on roster size and form. For demo purposes only.</div>
            </div>
          </Card>
        </div>

        {leagueId && (
          <div className="text-xs text-gray-600">Data: /lineups?team_id=..., /me/leagues, /leagues/{"{leagueId}"}/members</div>
        )}
      </div>
    </div>
  );
