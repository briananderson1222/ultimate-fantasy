  ...
  return (
    <Dialog.Root open={open} onOpenChange={(v) => !v && onClose()}>
      <Dialog.Portal>
        ...
        >
          <Dialog.Title id={titleId} className={title ? "mb-2 text-lg font-medium" : "sr-only"}>
            {title || "Modal"}
          </Dialog.Title>
          {children}
          <Dialog.Close aria-label="Close" className="absolute right-2 top-2 rounded-[var(--radius-sm)] px-2 py-1 text-xs text-[var(--color-muted)] hover:bg-[rgba(0,0,0,0.04)] focus-visible:ring-2 focus-visible:ring-[var(--ring)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--ring-offset)]">
            Close
          </Dialog.Close>
        </Dialog.Content>
      </Dialog.Portal>
