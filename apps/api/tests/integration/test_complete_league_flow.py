...
class TestCompleteLeagueFlow:
    ...
    def test_complete_league_creation_and_joining_flow(self):
        ...
        register_response = self.client.post(
            "/api/v1/auth/register", json=self.user1_data
        )
        ...
        league_response = self.client.post(
            "/api/v1/leagues", json=self.league_data, headers=commissioner_headers
        )
        ...
        register_response_2 = self.client.post(
            "/api/v1/auth/register", json=self.user2_data
        )
        ...
        join_data = {"invite_code": invite_code, "team_name": "My Fantasy Team"}
        join_response = self.client.post(
            f"/api/v1/leagues/{league_id}/join", json=join_data, headers=member_headers
        )
    ...
    def test_league_joining_with_invalid_invite_code_fails(self):
        ...
        join_data = {"invite_code": "INVALID_CODE", "team_name": "My Team"}
        response = self.client.post(
            "/api/v1/leagues/some_league_id/join", json=join_data, headers=headers
        )
    ...
    def test_league_member_limit_enforcement(self):
        ...
        # Create league
        league_response = self.client.post(
            "/api/v1/leagues", json=small_league_data, headers=headers
        )
    ...
    def test_duplicate_team_names_in_league_rejected(self):
        ...
        league_response = self.client.post(
            "/api/v1/leagues", json=self.league_data, headers=headers
        )
        league_id = league_response.json()["league_id"]
        invite_code = league_response.json()["invite_code"]
