...
class DatabaseManager:
        ...
        try:
            ...
            with self.session_scope() as session:
                if "postgresql" in self.config.database_url:
                    ...
                    """))
                    db_settings = {row[0]: f"{row[1]} {row[2] or ''}".strip()
            ...
            health_status['checks']['performance'] = {
                ...
                'metrics': {
                    ...
                    'avg_query_time_ms': round(
                        (self.stats['total_query_time'] / max(self.stats['query_count'], 1)) * 1000, 2
                    ),
    ...
    async def create_tables(self, metadata: Optional[MetaData] = None):
    ...
    async def drop_tables(self, metadata: Optional[MetaData] = None):
    ...
    async def run_health_diagnostics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                diagnostics['tests']['transactions'] = {'status': 'fail', 'error': str(e)}
            ...
            # Overall status
            failed_tests = sum(1 for test in diagnostics['tests'].values()
