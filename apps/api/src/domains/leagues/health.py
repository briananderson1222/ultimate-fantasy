...
async def leagues_detailed_health_check(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        branding_count = db.execute(text("SELECT COUNT(*) FROM league_branding")).scalar()
        detailed_status["checks"]["league_branding_table"] = {
            "status": "healthy",
        ...
        recent_leagues = db.execute(text("""
