...
class Achievement(Base):
    ...
    Implements T024 requirements:
    - Achievement entity for user milestones and badges
    - Add achievement_data JSON with progress tracking
    - Include rarity and unlock conditions
    """
    ...
    # Achievement context (optional league/team specific)
    ...
    # Achievement data and unlock conditions (JSON object)
    achievement_data: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON, nullable=True, default=dict)
    unlock_conditions: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON, nullable=True, default=dict)
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "progress_current >= 0",
            name="non_negative_progress_current"
        ),
        CheckConstraint(
            "progress_target > 0",
            name="positive_progress_target"
        ),
        CheckConstraint(
            "progress_current <= progress_target",
            name="progress_within_target"
        ),
        # Points value must be positive
        CheckConstraint(
            "points_value > 0",
            name="positive_points_value"
        ),
        # Display order must be non-negative
        CheckConstraint(
            "display_order >= 0",
            name="non_negative_display_order"
        ),
        # Unlocked achievements should have unlock timestamp
        CheckConstraint(
        ...
        # Composite indexes
        Index('idx_user_unlocked_achievements', 'user_id', 'is_unlocked', 'unlocked_at'),
        Index('idx_user_progress_achievements', 'user_id', 'is_unlocked', 'progress_current'),
        Index('idx_featured_achievements', 'is_featured', 'is_visible', 'display_order'),
    )

    def __repr__(self) -> str:
        return f"<Achievement(user_id='{self.user_id}', type='{self.achievement_type}', unlocked={self.is_unlocked})>"
    ...
    def get_condition_value(self, key: str, default: Any = None) -> Any:
        """Get specific unlock condition value"""
    ...
    def get_total_points(self) -> int:
        """Get total points including rarity multiplier"""
    ...
    def create_first_login_achievement(cls, user_id: _uuid.UUID) -> "Achievement":
        """Factory method for first login achievement"""
        return cls(
            user_id=user_id,
    ...
    @classmethod
    def create_first_league_achievement(cls, user_id: _uuid.UUID, league_id: _uuid.UUID) -> "Achievement":
        """Factory method for first league join achievement"""
        return cls(
            user_id=user_id,
            league_id=league_id,
    ...
    def create_championship_achievement(
        ...
        """Factory method for championship achievement"""
        return cls(
            user_id=user_id,
            league_id=league_id,
            team_id=team_id,
