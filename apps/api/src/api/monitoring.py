...
async def get_metrics_summary(
    ...
    error_rate = (metrics['errors_total'] / total_requests * 100) if total_requests > 0 else 0
    slow_request_rate = (metrics['slow_requests'] / total_requests * 100) if total_requests > 0 else 0
    ...
    success_requests = sum(count for status, count in status_codes.items() if 200 <= status < 300)
    client_error_requests = sum(count for status, count in status_codes.items() if 400 <= status < 500)
    server_error_requests = sum(count for status, count in status_codes.items() if status >= 500)
