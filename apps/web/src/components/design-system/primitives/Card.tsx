...
const cardVariants = cva("rounded-lg transition-all duration-200", {
  variants: {
    variant: {
      ...
      interactive:
        "bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md hover:border-gray-300 cursor-pointer",
...
const Card = React.forwardRef<HTMLDivElement, CardProps>(
  (
    ...
    const effectiveVariant =
      (onClick || clickable) && !disabled && variant !== "interactive" ? "interactive" : variant;
    ...
    return (
      <div
        ref={ref}
        className={cn(cardVariants({ variant: effectiveVariant, padding, disabled }), className)}
