...
export function EmptyState({ icon, title, description, action, className }: EmptyStateProps) {
  return (
    ...
    >
      ...
      <h3
        className="mb-2 text-lg font-medium text-[var(--color-text)] animate-fade-in"
        style={{ animationDelay: "var(--anim-delay-xs)" }}
      >
        {title}
      </h3>
      {description && (
        <p
          className="mb-6 max-w-sm text-[var(--color-muted)] animate-fade-in"
          style={{ animationDelay: "var(--anim-delay-sm)" }}
        >
          {description}
        </p>
      )}
