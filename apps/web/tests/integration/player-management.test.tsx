...
const mockPlayer = {
  ...
  stats: { points: 24.5, projected: 22.3 },
  injury: null,
...
const mockBenchPlayer = {
  ...
  stats: { points: 18.2, projected: 19.5 },
  injury: { status: "questionable" as const, description: "Minor ankle injury" },
  status: "active" as const,
  isStarter: false,
...
describe("Player Management Integration", () => {
  ...
  it("should distinguish between starters and bench players", () => {
    const { container: starterContainer } = renderWithTheme(<PlayerCard player={mockPlayer} />);
    const { container: benchContainer } = renderWithTheme(<PlayerCard player={mockBenchPlayer} />);
