...
export class OfflineSync {
  ...
  public async queueForSync(
    data: Omit<SyncableData, "id" | "lastModified" | "requiresSync">,
  ): Promise<void> {
    ...
    this.syncStatus.pendingSync = this.syncQueue.filter(
      (item) => item.requiresSync,
    ).length;
  ...
  private async syncItem(item: SyncableData, retryCount = 0): Promise<void> {
    try {
      ...
      console.error(
        `Failed to sync ${item.type} after ${this.config.maxRetries} retries:`,
        error,
      );
      this.syncStatus.syncErrors.push(
  ...
  private async syncLeague(item: SyncableData): Promise<void> {
    ...
    switch (action) {
      ...
      case "update":
        ...
        break;
      case "join":
        await this.apiClient.post(`/leagues/${leagueData.id}/join`, {
          userId: leagueData.userId,
        });
        break;
      case "leave":
        await this.apiClient.post(`/leagues/${leagueData.id}/leave`, {
          userId: leagueData.userId,
        });
        break;
    }
  ...
  private async syncPlayer(item: SyncableData): Promise<void> {
    ...
    switch (action) {
      case "add":
        await this.apiClient.post(
          `/teams/${playerData.teamId}/players`,
          playerData,
        );
        break;
      case "remove":
        await this.apiClient.delete(
          `/teams/${playerData.teamId}/players/${playerData.playerId}`,
        );
        break;
      case "trade":
  ...
  private async syncTrade(item: SyncableData): Promise<void> {
    ...
    switch (action) {
      case "propose":
        ...
        break;
      case "accept":
        await this.apiClient.post(`/trades/${tradeData.id}/accept`, {
          userId: tradeData.userId,
        });
        break;
      case "reject":
        await this.apiClient.post(`/trades/${tradeData.id}/reject`, {
          userId: tradeData.userId,
        });
        break;
      case "cancel":
