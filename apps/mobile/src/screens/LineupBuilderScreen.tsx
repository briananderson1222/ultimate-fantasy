...
export default function LineupBuilderScreen() {
  ...
  const lineupValidation = useMemo(() => {
    ...
    Object.entries(teamCounts).forEach(([team, count]) => {
      if (count > constraints.maxPlayersPerTeam) {
        issues.push(
          `Too many players from ${team} (${count}/${constraints.maxPlayersPerTeam})`,
        );
      }
    });
  ...
  const optimizationSuggestions = useMemo(() => {
    ...
    const wrs = lineup.filter(
      (slot) =>
        ["WR", "FLEX"].includes(slot.position) &&
    ...
    const injuredPlayers = lineup.filter(
      (slot) =>
        slot.player &&
        ["questionable", "doubtful"].includes(slot.player.status),
    );
  ...
  const assignPlayerToSlot = useCallback(
    (player: Player, slot: LineupSlot) => {
      if (!slot.acceptedPositions.includes(player.position)) {
        Alert.alert(
          "Invalid Position",
          `${player.name} cannot be assigned to ${slot.position}`,
        );
        return false;
      }
      ...
      if (existingSlot) {
        Alert.alert(
          ...
          `${player.name} is already in your lineup`,
        );
        return false;
      }

      setLineup((prev) =>
        prev.map((s) => (s.id === slot.id ? { ...s, player } : s)),
      );
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () => {
            ...
            const available = availablePlayers.filter(
              (p) => !lineup.some((slot) => slot.player?.id === p.id),
            );
          ...
          [
            {
              text: "Remove",
              style: "destructive",
              onPress: () => removePlayerFromSlot(slot.id),
            },
            {
              text: "Replace",
              onPress: () => {
          ...
          <View style={styles.slotPlayerInfo}>
            ...
            {slot.player.status !== "healthy" && (
              <View
                style={[
                  styles.statusBadge,
                  { backgroundColor: getStatusColor(slot.player.status) },
                ]}
              >
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text style={styles.headerSubtitle}>
            {lineupValidation.filledSlots}/{lineupValidation.totalSlots} players
            set
          </Text>
        </View>
      ...
      >
        <View style={styles.lineupGrid}>{lineup.map(renderLineupSlot)}</View>
      </ScrollView>
