...
export default function WaiverScreen() {
  ...
  const [waiverClaims, setWaiverClaims] =
    useState<WaiverClaim[]>(MOCK_WAIVER_CLAIMS);
  const [settings, setSettings] = useState<WaiverSettings>(MOCK_SETTINGS);
  const [refreshing, setRefreshing] = useState(false);

  // UI state
  const [currentTab, setCurrentTab] = useState<
    "available" | "claims" | "trends" | "settings"
  >("available");
  ...
  const filteredPlayers = useMemo(() => {
    return waiverPlayers.filter((player) => {
      ...
      if (filters.trending !== "ALL" && player.trending !== filters.trending)
        return false;
      if (
        player.ownership_percentage < filters.minOwnership ||
        player.ownership_percentage > filters.maxOwnership
      )
        return false;
      if (filters.opportunity !== "ALL") {
        const level =
          player.opportunity_score >= 75
            ? "high"
            : player.opportunity_score >= 50
              ? "medium"
              : "low";
  ...
  const pendingClaims = waiverClaims.filter(
    (claim) => claim.status === "pending",
  );
  const completedClaims = waiverClaims.filter(
    (claim) => claim.status !== "pending",
  );
  ...
  const createWaiverClaim = useCallback(() => {
    ...
    if (newClaim.bidAmount > settings.remaining_budget) {
      Alert.alert(
        "Insufficient Budget",
        "You don't have enough FAAB remaining for this bid.",
      );
      return;
    }
    ...
    setSettings((prev) => ({
      ...prev,
      remaining_budget: prev.remaining_budget - newClaim.bidAmount,
    }));
    ...
    Alert.alert(
      "Claim Submitted",
      `Your waiver claim for ${selectedPlayer.name} has been submitted.`,
    );
  }, [
    selectedPlayer,
    newClaim,
    settings.remaining_budget,
    pendingClaims.length,
  ]);

  const cancelWaiverClaim = useCallback(
    (claimId: string) => {
      Alert.alert(
        "Cancel Claim",
          ...
          { text: "No", style: "cancel" },
          {
            ...
            onPress: () => {
              ...
              if (claim) {
                ...
                setSettings((prev) => ({
                  ...prev,
                  remaining_budget: prev.remaining_budget + claim.amount,
                }));
                Alert.alert(
                  "Claim Cancelled",
                  "Your waiver claim has been cancelled.",
                );
              }
            },
          },
      ...
      <View style={styles.playerStats}>
        ...
        <View style={styles.statRow}>
          ...
          <Text
            style={[
              styles.statValue,
              { color: getOpportunityColor(player.opportunity_score) },
            ]}
          >
    ...
    <View style={styles.claimCard}>
      ...
      <View style={styles.claimDetails}>
        <Text style={styles.claimType}>
          {claim.claim_type === "add_drop" ? "Add/Drop" : "Add"} • $
          {claim.amount} FAAB
        </Text>
        {claim.drop_player && (
      ...
      <View style={styles.claimFooter}>
        <Text style={styles.claimTime}>
          {claim.status === "pending"
            ? `Processes in ${getTimeUntilProcess(claim.processes_at)}`
    ...
    >
      <View style={styles.modalContainer}>
        <View style={styles.modalHeader}>
          <Text style={styles.modalTitle}>Add Waiver Claim</Text>
            ...
            <>
              ...
              <View style={styles.claimSection}>
                ...
                <View style={styles.claimTypeOptions}>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setNewClaim((prev) => ({ ...prev, type: "add" }))
                    }
                  >
                    <Text
                      style={[
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setNewClaim((prev) => ({ ...prev, type: "add_drop" }))
                    }
                  >
                    <Text
                      style={[
                ...
                <View style={styles.bidSuggestions}>
                  {[5, 10, 15, 20].map((amount) => (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        setNewClaim((prev) => ({ ...prev, bidAmount: amount }))
                      }
                    >
              ...
              <View style={styles.claimSection}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) =>
                    setNewClaim((prev) => ({ ...prev, reason: text }))
                  }
                  multiline
                  numberOfLines={3}
                />
            ...
            <View style={styles.settingsSection}>
              ...
              <View style={styles.settingItem}>
                ...
                <Switch
                  ...
                  onValueChange={(value) =>
                    setSettings((prev) => ({ ...prev, auto_process: value }))
                  }
                />
              </View>
              <View style={styles.settingItem}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text style={styles.headerSubtitle}>
            ${settings.remaining_budget} budget • Priority #
            {settings.priority_number}
          </Text>
        </View>
