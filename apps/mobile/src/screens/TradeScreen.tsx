...
const MOCK_TRADES: Trade[] = [
  {
    ...
    ai_analysis: {
      ...
      reasoning: 'Josh Allen is valuable but CMC provides more positional scarcity at RB. Consider adding a WR2 to balance the trade.',
      confidence: 0.78
    },
...
export default function TradeScreen() {
  ...
  // UI state
  const [currentTab, setCurrentTab] = useState<'active' | 'history' | 'create' | 'market'>('active');
  ...
  const filteredTrades = useMemo(() => {
    return trades.filter(trade => {
      ...
      if (filters.team !== 'all' && trade.from_team !== filters.team && trade.to_team !== filters.team) return false;

      const tradeValue = (trade.offer_players.reduce((sum, p) => sum + p.trade_value, 0) +
                         trade.request_players.reduce((sum, p) => sum + p.trade_value, 0)) / 2;

      if (tradeValue < filters.minValue || tradeValue > filters.maxValue) return false;

      // Date range filtering
      if (filters.dateRange !== 'all') {
        const now = new Date();
        const daysDiff = Math.floor((now.getTime() - trade.created_at.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const activeTrades = filteredTrades.filter(t => ['pending', 'countered'].includes(t.status));
  const completedTrades = filteredTrades.filter(t => ['accepted', 'rejected', 'expired'].includes(t.status));

  // Trade actions
  const handleTradeAction = useCallback((trade: Trade, action: 'accept' | 'reject' | 'counter') => {
        ...
        { text: action.charAt(0).toUpperCase() + action.slice(1), onPress: () => {
          if (action === 'counter') {
            ...
            setTrades(prev => prev.map(t =>
              t.id === trade.id ? { ...t, status: action === 'accept' ? 'accepted' : 'rejected' } : t
            ));
            Alert.alert('Trade Updated', `Trade has been ${action}ed successfully.`);
          }
        }}
      ]
    );
  ...
  const createTrade = useCallback(() => {
    if (newTrade.myPlayers.length === 0 || newTrade.theirPlayers.length === 0) {
      Alert.alert('Invalid Trade', 'Please select players for both sides of the trade.');
      return;
    }
    ...
    setNewTrade({ targetTeam: '', myPlayers: [], theirPlayers: [], message: '' });
    ...
    >
      <View style={styles.tradeHeader}>
        ...
        <View style={styles.tradeStatus}>
          <View style={[styles.statusBadge, { backgroundColor: getTradeStatusColor(trade.status) }]}>
      ...
      {trade.ai_analysis && autoAnalyze && (
        <View style={styles.aiAnalysis}>
          ...
          <Text style={styles.aiRecommendation}>
            {trade.ai_analysis.recommendation.toUpperCase()}: {trade.ai_analysis.reasoning}
          </Text>
        </View>
      )}
    </TouchableOpacity>
  ...
  const renderTabBar = () => (
    <View style={styles.tabBar}>
      {[
        { key: 'active', label: 'Active', icon: 'time', badge: activeTrades.length },
    ...
    <FlatList
      ...
      ListEmptyComponent={
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtitle}>Create a trade to get started</Text>
        </View>
      }
    />
  );
  ...
  const renderCreateTab = () => (
    <ScrollView style={styles.createContainer} showsVerticalScrollIndicator={false}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {MOCK_TEAMS.filter(t => t.name !== 'My Team').map((team) => (
            <TouchableOpacity
              ...
              onPress={() => setNewTrade(prev => ({ ...prev, targetTeam: team.name }))}
            >
              <Text style={[
                styles.teamOptionText,
                newTrade.targetTeam === team.name && styles.selectedTeamOptionText
              ]}>
                {team.name}
              </Text>
      ...
      <View style={styles.createSection}>
        <Text style={styles.sectionTitle}>My Players ({newTrade.myPlayers.length})</Text>
        <Text style={styles.sectionSubtitle}>Select players you want to trade away</Text>
      ...
      <View style={styles.createSection}>
        <Text style={styles.sectionTitle}>Their Players ({newTrade.theirPlayers.length})</Text>
        <Text style={styles.sectionSubtitle}>Select players you want to receive</Text>
        {/* Player selection would be implemented here */}
        <TouchableOpacity style={styles.addPlayersButton}>
          <Ionicons name="add" size={20} color="#3b82f6" />
      ...
      <View style={styles.createSection}>
        ...
        <TextInput
          ...
          onChangeText={(text) => setNewTrade(prev => ({ ...prev, message: text }))}
          multiline
          numberOfLines={3}
        />
      </View>
      ...
      <TouchableOpacity
        style={[styles.createTradeButton, {
          opacity: newTrade.targetTeam && newTrade.myPlayers.length > 0 && newTrade.theirPlayers.length > 0 ? 1 : 0.5
        }]}
        onPress={createTrade}
        disabled={!newTrade.targetTeam || newTrade.myPlayers.length === 0 || newTrade.theirPlayers.length === 0}
      >
  ...
  const renderMarketTab = () => (
    <ScrollView style={styles.marketContainer} showsVerticalScrollIndicator={false}>
      <View style={styles.marketSection}>
        <Text style={styles.sectionTitle}>Market Insights</Text>
        {MOCK_MARKET_INSIGHTS.map((insight) => (
          <View key={insight.player_id} style={styles.insightCard}>
            <View style={styles.insightHeader}>
              <Text style={styles.insightPlayerName}>{insight.player_name}</Text>
              <View style={[
                styles.demandBadge,
                { backgroundColor: insight.demand_level === 'high' ? '#10b981' :
                                  insight.demand_level === 'medium' ? '#f59e0b' : '#6b7280' }
              ]}>
                <Text style={styles.demandText}>{insight.demand_level.toUpperCase()}</Text>
              </View>
            </View>
            <Text style={styles.insightText}>
              Avg Value: {insight.avg_value} • Recent Trades: {insight.recent_trades}
            </Text>
            <Text style={styles.insightText}>
              Price Trend: {insight.price_trend.charAt(0).toUpperCase() + insight.price_trend.slice(1)}
            </Text>
          </View>
        ))}
      </View>
  ...
  const renderContent = () => {
    switch (currentTab) {
      ...
      default: return renderActiveTab();
    }
  };
      ...
      {selectedTrade && (
        ...
        >
          <View style={styles.analyzerContainer}>
            ...
            <MobileTradeAnalyzer
              trade={selectedTrade}
              userTeam={{ id: 'team1', name: 'My Team', record: '7-3', rank: 3 }}
              teams={MOCK_TEAMS}
