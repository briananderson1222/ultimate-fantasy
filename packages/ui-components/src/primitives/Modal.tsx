...
export class Modal extends UIComponent<ModalProps> {
  getWebStyles() {
    return {
  ...
  componentDidMount() {
    if (PlatformAdapter.getCurrentPlatform() === "web" && this.props.open) {
  ...
  render() {
    ...
    if (PlatformAdapter.getCurrentPlatform() === "web") {
      const styles = this.getWebStyles();
    ...
    const styles = this.getNativeStyles();
